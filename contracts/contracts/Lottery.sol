...
import "hardhat/console.sol"; // To create gas usage report

/// @title Lottery
/// @author Irfan Bozkurt

/// @dev This contract implements a fully decentralized lottery logic that relies
/// @dev on non-malleable user commitments. This eliminates the dependency to on-chain
/// @dev RNG, offloading RNG completely to the end users. As a consequence,
/// @dev block-related information is not needed. This prevents fraudulent miners from
/// @dev manipulating the results.

/// @dev Each round consists of a purchase phase, and a reveal phase.
/// @dev Purchase phase is a one week period where users can buy a ticket to be a part
/// @dev of the on-going lottery at that time. Users must send a sha3 hash of two values
/// @dev encoded together: a random number, and their address.
/// @dev Reveal phase is another one week period where users are expected to reveal
/// @dev the random numbers they sent in phase 1. This reveal must be done from the
/// @dev very same address that made the cryptographic commitment in the previous week.

/// @dev Once these phases end users can make calls to the contract to see if they are
/// @dev one of the winners, or they can attempt claiming the reward immediately.

contract Lottery {
    event Log(bytes32 message); // For development purposes
    event TicketBought(
    ...
    uint32[2] private _cycleBreaker = [2147483647, 1963627463];
    ...
    function buyTicket(bytes32 hashRndNumber, uint8 ticketType) public {
        ...
        emit TicketBought(msg.sender, currentLotteryNo, ticketNo, ticketType);
    }
    ...
    function collectTicketRefund(uint256 ticketNo) public {
        ...
        uint8 ticketType = _lotteryRounds[lotteryNo]
            .tickets[ticketNo]
            .ticketType;
        uint256 ticketPrice = getTicketPrice(ticketType);
        ...
        emit TicketRefunded(msg.sender, lotteryNo, ticketNo, ticketType);
    }
    ...
    function revealRndNumber(uint256 ticketNo, uint256 rndNumber) public {
        ...
        if (
            keccak256(abi.encode(rndNumber, msg.sender)) ==
            _lotteryRounds[lotteryNo].tickets[ticketNo].hashRndNumber
        ) {
    ...
    function checkIfTicketWon(
        ...
        return
            _lotteryRounds[lotteryNo].yieldedAmounts[
                _getPlaceOfWinningTicket(lotteryNo, ticketNo) - 1
            ];
    }
    ...
    function getTotalLotteryMoneyCollected(
        ...
        require(
            lotteryNo <= currentLotteryInPurchase,
    ...
    function getLastOwnedTicketNo(
        ...
        require(
            lotteryNo > 0 && lotteryNo <= getCurrentLotteryInPurchase(),
            "Lottery round does not exist"
        );
        ...
        if (_isTicketWinner(lotteryNo, lastTicketNo)) {
            ...
            if (place != 0 && _lotteryRounds[lotteryNo].collected[place - 1])
                status = 2;
        }
    ...
    function getIthOwnedTicketNo(
        ...
        require(
            i > 0 && i <= _lotteryRounds[lotteryNo].ticketCount,
    ...
    function getLotteryNos(
        uint256 unixTime
    ) public view returns (uint256, uint256) {
        require(
            unixTime >= startTimeOfContract,
            "You're giving a very old date."
        );
    ...
    function getYieldedAmounts(
        ...
        for (uint256 i = 0; i < 3; i++)
            result[i] = _lotteryRounds[lotteryNo].yieldedAmounts[i];
    ...
    function _getTotalPrize(uint256 lotteryNo) internal returns (uint256) {
        ...
        for (uint256 i = lastCollectedCarryOut + 1; i < lotteryNo; i++)
            totalPrize += _lotteryRounds[i].carryOut;
    ...
    function _populateWinners(uint256 lotteryNo) internal {
        for (uint256 i = lastPopulatedWinners + 1; i <= lotteryNo; i++)
            __populateWinners(i);
    ...
    function __populateWinners(uint256 lotteryNo) internal {
        ...
        if (sampleSize > 0) {
            ...
            uint256 firstPlaceTicketId = _lotteryRounds[lotteryNo]
                .revealedTickets[firstPlaceIndex];
            _lotteryRounds[lotteryNo].winners[0] = firstPlaceTicketId;

            uint8 ticketType = _lotteryRounds[lotteryNo]
                .tickets[firstPlaceTicketId]
                .ticketType;
        ...
        if (sampleSize > 1) {
            _lotteryRounds[lotteryNo].xor ^= _cycleBreaker[0];
            ...
            uint256 secondPlaceTicketId = _lotteryRounds[lotteryNo]
                .revealedTickets[secondPlaceIndex];
            _lotteryRounds[lotteryNo].winners[1] = secondPlaceTicketId;

            uint8 ticketType = _lotteryRounds[lotteryNo]
                .tickets[secondPlaceTicketId]
                .ticketType;
            uint256 prize = _calculatePrizeByPosition(
                totalPrize,
                2,
                _lotteryRounds[lotteryNo]
                    .tickets[secondPlaceTicketId]
                    .ticketType
            );
    ...
    function _getPlaceOfWinningTicket(
        uint256 lotteryNo,
        uint256 ticketNo
